...
export const load: PageLoad = async ({ fetch, params, depends, route }) => {
	...
	if (!albumRes.ok) throw error(albumRes.status, 'Failed to load album!');

	const albumJSON:SpotifyApi.SingleAlbumResponse = await albumRes.json();

	let color=null;
	if(albumJSON.images.length > 0){
		const colorRes = await fetch(`/api/average-color?${new URLSearchParams({image: albumJSON.images[0].url}).toString()}`);
